...
from smart_manager.constants import SYSTEM_POWER, TaskInfo, CRONTAB_FILE
    ...
    def _refresh_crontab():
        ...
        with open(npath, "w") as cfo:
            ...
            for td in TaskDefinition.objects.filter(enabled=True):
                tinfo = TaskInfo(td.task_type)
                if td.crontab is not None:
                    tab = f"{td.crontab} root"
                    if td.task_type == "snapshot":
                        tab = f"{tab} {tinfo.script} {td.id}"
                    elif td.task_type == "scrub":
                        tab = (
                            f"{tab} {tinfo.script} {td.id}"
                        )
                    elif td.task_type in SYSTEM_POWER:
                        tab = f"{tab} {tinfo.script} {td.id}"
                    else:
